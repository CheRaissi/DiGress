...
class DivideTreeVisualization:
    ...
    def visualize_tree(self, graph, pos, path, iterations=100, node_size=100, largest_component=False):
        if largest_component:
            CGs = [graph.subgraph(c) for c in nx.connected_components(graph)]
            CGs = sorted(CGs, key=lambda x: x.number_of_nodes(), reverse=True)
            graph = CGs[0]
        ...
        # Visualiza peaks and saddles
        # extract peaks and saddles based on predicted symbol value
        peak_nodes = [n for (n,ty) in nx.get_node_attributes(graph,'symbol').items() if ty == 1.0]
        saddle_nodes = list(set(graph.nodes()) - set(peak_nodes) )

        plt.figure()

        nx.draw_networkx_nodes(graph, pos, nodelist=peak_nodes, node_color='red', node_shape='^')
        nx.draw_networkx_nodes(graph, pos, node_size=200, nodelist=saddle_nodes, node_color='blue', node_shape='o')
    ...
    def visualize(self, path: str, graphs: list, num_graphs_to_visualize: int, log='graph', trainer=None):
        ...
        for i in range(num_graphs_to_visualize):
            file_path = os.path.join(path, 'graph_{}.png'.format(i))
            graph = self.to_networkx(graphs[i][0].numpy(), graphs[i][1].numpy())
            self.visualize_tree(graph=graph, pos=None, path=file_path)
            im = plt.imread(file_path)
            wandb.log({log: [wandb.Image(im, caption=file_path)]})
    ...
    def visualize_chain(self, path, nodes_list, adjacency_matrix, trainer=None):
        ...
        for frame in range(num_frams):
            file_name = os.path.join(path, 'fram_{}.png'.format(frame))
            self.visualize_tree(graph=graphs[frame], pos=final_pos, path=file_name)
            save_paths.append(file_name)
